// Dynamic landing route: 24 SSG pediatric pages — Siraa brand palette.
// Desktop: two-column grid with sticky right form. Mobile: diagonal CTA bar + popup modal (pb-28 clearance).
...
import { LandingFAQ, DesktopStickyForm, MobileCTABar, MobileBookSection } from "@/components/landing/LandingClient";
...
const Section = ({ id, title, eyebrow, tone, children }: {
  id?: string; title?: string; eyebrow?: string; tone?: "tint"; children: React.ReactNode;
}) => (
  <section id={id} className={`py-10 md:py-14 ${tone === "tint" ? "bg-[#FDF0F0]/60 rounded-3xl px-5 md:px-8" : ""}`}>
...
export default async function LandingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="bg-[#F5F0FC] text-[#2d2d2d]">
      {schemas.map((s, i) => (
      ...
      <div className="text-white" style={{ background: "linear-gradient(135deg, #6B5B95, #8B6BAF)" }}>
        <div className="mx-auto max-w-6xl px-5 py-14 md:py-20">
          ...
          <h1 className="text-3xl md:text-5xl font-semibold leading-tight text-[#ffffff] max-w-3xl">{page.h1}</h1>
          <p className="mt-4 text-lg text-white/90 max-w-2xl">{page.heroSub}</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <a href="#book" className="inline-flex items-center justify-center rounded-full bg-[#2DBF6E] hover:bg-[#22A05A] text-white font-semibold min-h-[48px] px-6">{page.cta}</a>
          ...
          <ul className="mt-8 flex flex-wrap gap-2" aria-label="Why parents trust Siraa Health">
            {["MD Pediatricians", "4870+ Families Trust Us", "IAP Vaccination Schedule", "Same-Day Appointments"].map((t) => (
              <li key={t} className="rounded-full bg-white/15 text-sm font-medium px-4 py-2 flex items-center gap-2">
                <span aria-hidden="true" className="w-2 h-2 rounded-full bg-[#2DBF6E]" />{t}
              </li>
            ))}
          </ul>
        </div>
      ...
      {/* Two-column: content left, sticky form right (desktop) */}
      <div className="mx-auto max-w-6xl px-5 pb-28 lg:pb-16 lg:grid lg:grid-cols-[1fr_380px] lg:gap-10 lg:items-start">
        <div>
          ...
          <Section title={page.symptomsTitle} tone="tint"><Check items={page.symptoms} /></Section>
          ...
          <Section title={page.causesTitle} tone="tint">
            ...
            <ol className="space-y-4">
              {page.treatment.map((s, i) => (
                <li key={s.title} className="flex gap-4 rounded-2xl bg-white border border-[#EDD8F0] p-5">
                  <span className="shrink-0 w-9 h-9 rounded-full bg-[#E8614A] text-white font-semibold grid place-items-center" aria-hidden="true">{i + 1}</span>
                  <div><p className="font-semibold text-[#1a1a1a]">{s.title}</p><p className="text-[#2d2d2d]">{s.desc}</p></div>
                </li>
              ))}
            </ol>
            {/* Doctor quote inline on mobile only (desktop shows it in sticky column) */}
          ...
          <Section title={page.homeCareTitle} tone="tint"><Check items={page.homeCare} /></Section>

          <Section title="Frequently Asked Questions"><LandingFAQ faqs={page.faqs} /></Section>

          {/* Mobile in-page booking (desktop uses the sticky column) */}
          <Section id="book" eyebrow="Same-day slots for sick children">
            ...
            <div className="mt-5 rounded-2xl bg-white border border-[#EDD8F0] p-6">
              <p className="font-semibold text-[#1a1a1a] mb-2">Visit us</p>
              <p className="text-[#666666] text-sm">{LANDING_SITE.address}</p>
              <p className="text-[#666666] text-sm mt-1">{LANDING_SITE.hours}</p>
          ...
          <Section title="Related Care at Siraa">
            <ul className="flex flex-wrap gap-3">
              {page.related.map((r) => {
                ...
                return rp ? (
                  <li key={r}>
                    <Link href={r} className="inline-block rounded-full border-2 border-[#6B5B95] text-[#1a1a1a] font-semibold px-5 py-2.5 hover:bg-[#F5F0FC]">
                      {rp.nav} →
                    </Link>
                  </li>
                ) : null;
